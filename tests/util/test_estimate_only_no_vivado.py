...
def test_estimate_only_steps_defined():
    """Verify estimate_only_dataflow_steps stops before any hardware generation."""
    steps = build_cfg.estimate_only_dataflow_steps
    assert "step_generate_estimate_reports" in steps
    assert not any(
        keyword in step for step in steps for keyword in ("hardware", "stitched", "synth", "fifo")
    )
...
def test_estimate_only_flow_produces_reports():
    ...
    model_file = str(
        Path(__file__).parent.parent / "example_data" / "build_dataflow" / "model.onnx"
    )
