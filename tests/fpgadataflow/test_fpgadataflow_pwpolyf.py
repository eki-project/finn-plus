...
def insert_and_set_fifo_depths(model: ModelWrapper, fpga_part: str, clk_ns: float) -> ModelWrapper:
    ...
    model = model.transform(BuildSimulation(fpga_part, clk_ns, True, performance_sim=False))
...
def test_fpgadataflow_pwpolyf_rtl_constants_match():
    ...
    rtl_path = os.path.join(get_settings().finn_rtllib, "pwpolyf", "hdl", "pwpolyf.sv")
