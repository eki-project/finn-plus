...
def test_finnloop_end2end_mlo(
    ...
    # debug_fifo makes the (behavioral) rtlsim verification write per-FIFO transaction
    # logs. For MLO the loop-body FIFOs are tagged with their enclosing FINNLoop name.
    if run_fifo_debug:
        fifo_debug_dir = tmp_output_dir + "/debug/fifo_logs"
        assert os.path.isdir(fifo_debug_dir), f"missing fifo debug dir {fifo_debug_dir}"
        fifo_logs = [f for f in os.listdir(fifo_debug_dir) if f.endswith(".log")]
        assert len(fifo_logs) > 0, f"no per-FIFO debug logs in {fifo_debug_dir}"
        assert any(
            f.startswith("FINNLoop_0_") for f in fifo_logs
        ), f"loop-body fifo logs not tagged with loop context: {fifo_logs}"
