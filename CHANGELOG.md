...
- **FIFO sizing on a formal model of the accelerator**: two new `auto_fifo_strategy` options built on a timed-event-graph (TEG) model of the operator control paths instead of RTL simulation
    ...
    - New package `finn.analysis.fpgadataflow.teg`: model (`Chain`, `FIFOEdge`, `Arc`, freeze groups for HLS pipelines), simulator, MILP, search, `from_onnx` model construction and operator templates transcribed from the RTL/HLS sources (Thresholding RTL/HLS, MVAU/VVAU HLS, StreamingDataWidthConverter RTL/HLS, ConvolutionInputGenerator RTL default template incl. depthwise, Pool/GlobalAccPool/LabelSelect HLS, Reshape RTL); HLS stage latencies, register slices and rewind delays are read from the Vitis synthesis reports and schedule database (`teg.hls_report`)
    - Build options `teg_minimization_orders`, `teg_max_frames`, `teg_target_interval`, `teg_dump_model`
    - Differential tests (`tests/fpgadataflow/teg`, marker `fifo_model`) drive every operator template and the real RTL in XSI with identical randomised stall patterns and require identical handshake traces; end-to-end tests compare against recorded `distributed_sim` reports of the CI regression models. The CI test suite variant `fifo_model_ci` (`TEST_SUITE` pipeline variable, `finn test --variant fifo_model_ci`) runs only these tests and skips the benchmark jobs
