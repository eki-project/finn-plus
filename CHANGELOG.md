...
- CI test suite variants (`quicktest_ci`, `full_ci`) abort a single test after 3 hours via `pytest-timeout` in `thread` mode (stack dump, worker killed, crash rerun) instead of letting a test hung inside a C extension idle the whole job into the Slurm time limit
