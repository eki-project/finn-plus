...
- (Xilinx) Second upstream sync up to Xilinx#1703 (2026-09-23): `Requant_rtl` supports `mem_mode=internal_decoupled` with memstreamed scale and bias, also inside `FINNLoop` (Xilinx#1657); `Lookup` minimizes its embedding datatype and `GlobalAccPool` its output datatype (Xilinx#1701); `InferRequantLayer` skips signed-output `MultiThreshold` nodes with `out_bias == 0` and points to `AbsorbScalarBiasIntoMultiThreshold` (Xilinx#1699); `OuterShuffle` cycle estimation works without Vivado (Xilinx#1688); estimate-only regression test (Xilinx#1691)

### Changed
- (Xilinx) **FIFO consolidation** (Xilinx#1658, Xilinx#1679): all RTL FIFOs are built from a single `finn-rtllib/fifo/hdl/fifo.sv` (SRL / LUTRAM / BRAM / URAM selected by `ram_style`, new value `srl`). The Vivado `axis_data_fifo` implementation (`impl_style=vivado`), `SplitLargeFIFOs` and the `split_large_fifos` build config option are removed; FIFOs of any depth are built as one instance. The FIFO gauge counts 32 bit with an overflow sentinel. The distributed simulation no longer models per-sub-FIFO pipeline overhead.
...
- (Xilinx) DWC consolidation (Xilinx#1660): the RTL DWC (`finn-rtllib/dwc/hdl/vpc.sv`) handles any input/output width ratio, so the integer-ratio restriction at specialization is gone; AXI-Lite write data is read 2-state in rtlsim (`Port.as_hexstr_2state`)
...
- (Xilinx) `finn-rtllib/swg` is identical to upstream again: the depthwise-mode deadlock fix and counter width tightening of Xilinx#1698 supersede the FINN+ fix (#264)
- Not pulled from upstream: the SLASH/V80 linker (`alveo_build.py`), phase-based build steps, `build_dataflow_checks` (including the folding-config/target-fps and verification-prerequisite checks of Xilinx#1665/Xilinx#1673), the Jenkins CI package
- (Xilinx) RTL MVAU is selected for datatypes wider than 8 bit as well (Xilinx#1568), bounded by the DSP datapath widths at specialization and in the RTL code generation (activations on the signed B port: 18/24 bit, one less for unsigned; weights 25/27 bit; accumulator 48/58 bit; `get_dsp_datapath_limits`, Xilinx#1696)
...
- `Pool` minimizes its accumulator/output datatype (`AccPool`/`AvgPool`; `AccumBits` of `QuantAvgPool` as in Xilinx#1701) and passes the input datatype through for `MaxPool` when datatypes change after conversion (Xilinx#1697)
- CI test suite variants (`quicktest_ci`, `full_ci`) abort a single test after 3 hours via `pytest-timeout` in `thread` mode (worker killed, crash rerun), preceded by a `faulthandler` stack dump of the hung test, instead of letting a test hung inside a C extension idle the whole job into the Slurm time limit
- New pytest marker `xslow` for tests that take hours each; the CI test variants deselect it. The upstream FINNLoop end-to-end grid (34 cases of one to two and a half hours) runs a representative subset of four by default, the rest is `xslow`
- Build config option `large_fifo_mem_style` is back (`auto`/`block`/`distributed`/`ultra`): it pins the memory style of the FIFOs `fifo.sv` would back with BRAM/URAM after sizing, since the RTL's `auto` takes URAM for every FIFO deeper than 2028 entries (the ResNet-18 regression build over-utilized the 80 URAMs of the RFSoC2x2 with 96 once the FIFOs joined the weight memories)
...
- The per-node projects of the FIFO-sizing simulation launched all their Vivado synthesis runs at once (`-jobs` = FINN workers), so every parallel node build could run several `synth_design` processes and exceed the 10 GB per build that the Slurm worker cap assumes; after the upstream merge added cells this got runs OOM-killed in the transformer regression builds. They now synthesize with two jobs (`CreateStitchedIP(synth_jobs=...)`). `Reshape_rtl` referenced the DWC source that upstream renamed to `vpc.sv` (Xilinx#1660)
