    ...
    def apply(self, model : ModelWrapper):
        ...
        for n in graph.node:
            ...
            if n.op_type == "Quant":
                ...
                print(f"conv input is {target_node.input}")
                ...
                print(f"conv node {target_node.name}")
                print(f"quant output shape {q_node_output.shape}")
                # For both mul and Add:
                # Move the scale factor behind the next operator
                scale = model.get_initializer(n.input[1])
                new_initializer = q_node_output / scale
 
                print(f"new init is {new_initializer}")
                ...
                print(f"add init shape {new_initializer.shape}")
                print(f"add init shape {new_initializer}")
                ...
                old_output_shape = model.get_tensor_shape(successor.output[0])
                ...
                act_div_tensor = helper.make_tensor_value_info(
                    model.make_new_valueinfo_name(),
                    TensorProto.FLOAT,
                    old_output_shape,  # pass the old shape here
                )
                ...
                # model.save("models/metaFi_test1.onnx")
                # cleanup_model(model)
                # return model
                # break
