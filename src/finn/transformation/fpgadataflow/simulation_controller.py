    ...
    def _run_binary(
        ...
        def _print(msg: str, color: str = "green") -> None:
            if self.progress is None:
                if is_end_node:
                    color = "orange3"
                self.console.log(f"[bold {color}]{name:<35}[/bold {color}] {msg:<35}")

        ld_library_path = "LD_LIBRARY_PATH=" + get_vivado_root() + "/lib/lnx64.o:$LD_LIBRARY_PATH"
        cwd = binary.parent
        if name is None:
        ...
        try:
            ...
            _send = lambda cmd: self._send(proc, cmd)  # noqa: E731
            received = ""
            while received != "end":
                ...
                elif received[1] == "stopped":
                    with self.running_lock:
                        self.running -= 1
                        _print(f"Running: {self.running} / {self.total}")
                else:
                    raise FINNInternalError(f"Simulation {name}: Unrecognized command: {received}")
