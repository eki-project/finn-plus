"""Unsqueeze hardware custom operator (ONNX ``Unsqueeze``, folding-aware passthrough)."""
...
class Unsqueeze(HWCustomOp):
    """Hardware custom operator for the Unsqueeze operation.
    ...
    def make_shape_compatible_op(self, model: ModelWrapper) -> NodeProto:  # noqa: ARG002
        ...
        # Though providing unsqueezed axes via a second input is supported by the
    ...
    def execute_node(
        ...
        """Execute the unsqueeze operation (Python fallback)."""
        node = self.onnx_node
        ...
        # input to the operator, this input specifies the axes to be unsqueezed
        if axes is None and len(node.input) > 1:
            axes = context[node.input[1]]
        if axes is None:
            raise FINNInternalError(
                f"{node.name}: Unsqueeze requires axes as an attribute or a second input"
            )
        axis = tuple(cast("list[int] | np.ndarray", axes))
        out = np.expand_dims(inp, axis=axis)
