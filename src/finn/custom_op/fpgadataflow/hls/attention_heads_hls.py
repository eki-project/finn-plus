...
class SplitMultiHeads_hls(SplitMultiHeads, HLSBackend):
    ...
    def _execute_node_cppsim(
        ...
        np.save(Path(code_gen_dir) / "in.npy", inp)
        ...
        for i, name in enumerate(node.output):
            ...
            out = np.load(Path(code_gen_dir) / f"out{i}.npy")
    ...
    def docompute(self) -> None:
        ...
        self.code_gen_dict["$DOCOMPUTE$"] = [
            ...
            *(f"{out(i)}.write(x{split(i)});" for i in range(self.heads)),
            ...
            "}"  # Note: closing brace for the loop opened above
        ]
    ...
    def blackboxfunction(self) -> None:
        ...
        self.code_gen_dict["$BLACKBOXFUNCTION$"] = [
            ...
            f"IStream &in_{self.hls_sname()}, ", ",".join([
                # One output HLS stream per head
                f"OStream &out{i}_{self.hls_sname()}" for i in range(self.heads)
            ]),
            ")",
            # @formatter:off
...
class MergeMultiHeads_hls(MergeMultiHeads, HLSBackend):
    ...
    def _execute_node_cppsim(
        ...
        for i, name in enumerate(node.input):
            ...
            # Save the folded inputs to file to be used by simulation
            np.save(Path(code_gen_dir) / f"in{i}.npy", inp)

        # Execute the precompiled model
        super().exec_precompiled_singlenode_model()

        # Load the output numpy file generated by the C++ simulation
        out = np.load(Path(code_gen_dir) / "out.npy")
        # Reshape the folded output and insert into the execution context
