...
def parse_dataflow_build_config(content: str) -> Dict[str, Any]:
    ...
    for node in dataflow_config.body:
        if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
            ...
            # Get type annotation
            type_annotation: str = ast.unparse(node.annotation) if hasattr(
            ...
            if node.value:
                if isinstance(node.value, ast.Constant):
                    ...
                    default_value = ast.unparse(node.value) if hasattr(
...
def generate_markdown_documentation(config_data: Dict[str, Any], output_file: str) -> None:
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("## Summary\n\n")
        f.write(
            f"- **Total Configuration Fields:** {len(config_data['fields'])}\n")
...
def main() -> int:
    ...
    parser.add_argument('--input', '-i', default="../src/finn/builder/build_dataflow_config.py",
                        help='Path to build_dataflow_config.py file')
    parser.add_argument('--output', '-o', default="DataflowBuildConfig_Documentation.md",
    ...
    try:
        ...
        undocumented_fields: List[str] = [
            field['name'] for field in config_data['fields']
