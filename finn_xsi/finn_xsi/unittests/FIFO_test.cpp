...
TEST_F(FIFOTest, ConstructorWithDefaultSize) {
  ...
  EXPECT_FALSE(fifo.isOutputValid());
}
...
TEST_F(FIFOTest, PushOneElement) {
  ...
  EXPECT_FALSE(fifo.isEmpty());
  EXPECT_TRUE(fifo.isOutputValid());
  EXPECT_TRUE(fifo.isInputReady());
...
TEST_F(FIFOTest, TryPushAlternatingValid) {
  ...
  for (int i = 0; i < 10; ++i) {
    fifo.tryPush(i % 2 == 0); // Push only on even iterations
    fifo.toggleClock();
  }
...
TEST_F(FIFOTest, TryPopAlternatingReady) {
  ...
  // Pop alternating
  for (int i = 0; i < 10; ++i) {
    fifo.tryPop(i % 2 == 0); // Pop only on even iterations
    fifo.toggleClock();
  }
...
TEST_F(FIFOTest, TryMethodsVsUpdateEquivalence) {
  ...
  EXPECT_EQ(fifo1.isOutputValid(), fifo2.isOutputValid());
}
...
TEST_F(FIFOTest, TryMethodsStressTest) {
  ...
  // Complex pattern
  for (int i = 0; i < 500; ++i) {
    fifo.tryPush(i % 3 != 0); // Push 2 out of 3 times
    if (i > 100) {
      fifo.tryPop(i % 2 == 0); // Pop every other time after 100
    }
    fifo.toggleClock();
  }
  ...
  EXPECT_EQ(fifo.size() > 0, fifo.isOutputValid());
}
