...
- `steps`: the list of build steps to run. This example appends the custom step `custom_step_gen_tb_and_io` to `default_build_dataflow_steps`. Alternatively, `inject_steps_after` / `inject_steps_before` accept dicts mapping a step name to a list of custom step functions to run after/before it, e.g. `inject_steps_after={"step_deployment_package": [custom_step_gen_tb_and_io]}`, without having to redefine the whole `steps` list.
...
```
   ...
   Running step: step_create_stitched_ip [14/21]
   Running step: step_measure_rtlsim_performance [15/21]
   Running step: step_out_of_context_synthesis [16/21]
   Running step: step_prepare_synthesis [17/21]
   Running step: step_synthesize_bitfile [18/21]
   Running step: step_make_driver [19/21]
   Running step: step_deployment_package [20/21]
   Running step: custom_step_gen_tb_and_io [21/21]
   Completed successfully
```

This build runs the default step list and adds a custom trailing step,
`custom_step_gen_tb_and_io`, which generates the testbench data. Steps that are not
needed for the requested outputs (e.g. bitfile synthesis or the driver) are skipped
automatically.
